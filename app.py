...
def get_weather_data(api_key: str, location: str, start_date: str, end_date: str) -> dict:
    ...
    logging.debug(f"Requesting weather data from url: {base_url}")
    retries = 5
    ...
    logging.error("Failed to retrieve weather data after multiple attempts.")
    return {}
...
def index():
    ...
    if request.method == "POST":
        ...
        budget = request.form.get("budget", '')  # Default to empty string if not provided
        trip_type = request.form.get("trip_type", 'solo')  # Default to 'solo' if not provided
        transport_mode = request.form.get("transport_mode", 'train')
        ...
        try:
            plan = bard.generate_itinerary(source, destination, start_date, end_date, no_of_day, budget, trip_type, transport_mode)
